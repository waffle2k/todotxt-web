...
BASE_URL  = os.environ.get("TODO_URL",  "http://localhost:5000")
USERNAME  = os.environ.get("TODO_USER", "")
PASSWORD  = os.environ.get("TODO_PASS", "")
...
def _login():
    ...
    if not USERNAME or not PASSWORD:
        _die("Set TODO_USER and TODO_PASS environment variables.")
    r = _session.post(
        f"{BASE_URL}/login",
    ...
    if not _authenticated:
        _die("Login failed — check TODO_USER and TODO_PASS.")
...
def _get(path, **kwargs):
    ...
    if not _authenticated:
        _login()
    r = _session.get(f"{BASE_URL}{path}", allow_redirects=True, **kwargs)
    if "/login" in r.url:
        _login()
        r = _session.get(f"{BASE_URL}{path}", allow_redirects=True, **kwargs)
    return r
...
def _post(path, data):
    ...
    if not _authenticated:
        _login()
    r = _session.post(f"{BASE_URL}{path}", data=data, allow_redirects=True)
    if "/login" in r.url:
        _login()
        r = _session.post(f"{BASE_URL}{path}", data=data, allow_redirects=True)
    return r
...
{SHORTHELP}
  ...
  Environment:
    TODO_URL   Base URL  (default: http://localhost:5000)
    TODO_USER  Username
    TODO_PASS  Password
